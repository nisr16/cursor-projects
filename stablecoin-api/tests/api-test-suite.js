...
/**
 * Test Central American countries bank registration
 * This ensures that all Central American countries are properly supported
 */
async function testCentralAmericanCountries() {
  ...
  for (const country of centralAmericanCountries) {
    log(`Testing ${country} Bank Registration...`);
    
    const bankData = {
      bank_name: `${country} Bank ${Date.now()}`,
      bank_code: `${country.substring(0, 2).toUpperCase()}${Date.now()}`,
      contact_email: `${country.toLowerCase().replace(' ', '')}${Date.now()}@testbank.com`,
      country: country
    };
    
    const result = await makeRequest('POST', '/api/banks/register', bankData);
    
    assert(result.success, `${country} bank registration should succeed`);
    assert(result.status === 201, `${country} bank registration should return 201`);
    assert(result.data.bank, `${country} bank registration should return bank data`);
    assert(result.data.bank.country === country, `Bank country should be ${country}`);
    assert(result.data.api_credentials, `${country} bank registration should return API credentials`);
    
    log(`${country} bank registration test passed`, 'success');
    testResults.passed++;
  }
}
